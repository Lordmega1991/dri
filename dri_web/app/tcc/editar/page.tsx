...
import { fetchSigaaStudents } from '../cadastrar/actions'

function EditarDefesaContent() {
        ...
        try {
            ...
            if (data) {
                setFormData({
                    ...
                    dia: data.dia || '',
                    hora: data.hora || '',
                    discente: data.discente || '',
                    matricula: data.matricula?.toString() || '',
                    orientador: data.orientador || '',
                    coorientador: data.coorientador || '',
                    avaliador1: data.avaliador1 || '',
                    instituto_av1: data.instituto_av1 || '',
                    avaliador2: data.avaliador2 || '',
                    instituto_av2: data.instituto_av2 || '',
                    avaliador3: data.avaliador3 || '',
                    instituto_av3: data.instituto_av3 || '',
                    titulo: data.titulo || '',
                    local: data.local || '',
                    login: data.login || '',
                    senha: data.senha || ''
            ...
            console.error('Erro ao buscar dados da defesa:', err)
            alert('Erro ao carregar dados da defesa')
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (error) throw error
            alert('Defesa atualizada com sucesso!')
            router.push('/tcc/visualizar')
        } catch (err) {
            console.error(err)
            alert('Erro ao atualizar defesa')
        } finally {
            setLoading(false)
        }
    }
            ...
            <header className="bg-white border-b border-slate-100 sticky top-0 z-40">
                <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href="/tcc/visualizar" className="p-2 hover:bg-slate-50 rounded-full transition-colors text-slate-400">
                            <ArrowLeft size={18} />
                        </Link>
                        <h1 className="text-lg font-bold text-slate-800 tracking-tight italic">Editar Defesa</h1>
                    </div>
                    <button
                        form="defense-form"
                        disabled={loading}
                        className="bg-[#0C4A6E] hover:bg-[#0284C7] text-white px-6 py-2 rounded-full font-bold text-sm transition-all shadow-lg shadow-indigo-100 disabled:opacity-50 flex items-center gap-2"
                    >
                        ...
                        <div className="flex items-center justify-between mb-2">
                            <div>
                                ...
                                <p className="text-[11px] font-medium italic text-emerald-500 mt-1">
                                    Credenciais para os avaliadores externos acessarem a banca.
                                </p>
                            </div>
                            <div className="flex items-center gap-2">
                                <button type="button" onClick={generateCredentials} className="text-emerald-600 hover:text-emerald-800 p-2 hover:bg-emerald-100 rounded-full transition-all" title="Regerar Credenciais">
                                    <RefreshCw size={18} />
                                </button>
                                <button type="button" onClick={copyCreds} className="text-emerald-600 hover:text-emerald-800 p-2 hover:bg-emerald-100 rounded-full transition-all" title="Copiar">
                                    <Copy size={18} />
                                </button>
                            </div>
                        </div>
...
export default function EditarDefesaPage() {
    return (
