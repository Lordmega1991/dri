...
export default function MinhasBancasPage() {
    ...
    const fetchDefesas = async (userName: string) => {
        setLoading(true)
        try {
            // 1. Fetch Defesas (already filtered by user name for better performance if possible, 
            // but multiple OR conditions across columns is easier in JS for now or complex RPC)
            const { data: defesasData, error: defesasError } = await supabase
                .from('dados_defesas')
                .select('*')
            ...
            if (defesasData) {
                // Filter only defenses where user participates
                const myBancas = defesasData.filter(d => {
                    const roles = [
                        d.orientador,
                        d.coorientador,
                        d.avaliador1,
                    ...
                    return roles.includes(userName.toLowerCase().trim())
                })
    ...
    const handleGenerateAta = async () => {
        ...
        try { await generateAta(selectedDefesa as any); }
    ...
    const handleGenerateFolhaAprovacao = async () => {
        if (!selectedDefesa) return;
        setGeneratingPdf(true);
        try { await generateFolhaAprovacao(selectedDefesa as any); }
    ...
    const handleGenerateFichaFinal = async () => {
        if (!selectedDefesa) return;
        setGeneratingPdf(true);
        try { await generateFichaFinal(selectedDefesa as any); }
            ...
            <div className="bg-[#E0F2FE] rounded-b-[2rem] shadow-sm pb-4">
                <div className="max-w-7xl mx-auto px-4 pt-4 flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-[#0C4A6E] text-base font-black italic">Minhas Bancas</h1>
                            <p className="text-[10px] text-[#0284C7] font-bold uppercase tracking-widest -mt-1">PARTICIPAÇÃO NOS TCC</p>
                        </div>
                    </div>
                    <button onClick={() => user?.user_metadata?.full_name && fetchDefesas(user.user_metadata.full_name)} className="w-8 h-8 bg-white rounded-lg shadow-sm flex items-center justify-center text-[#0C4A6E] hover:bg-slate-50 transition-colors">
                ...
                <div className="max-w-7xl mx-auto px-4 space-y-2">
                    ...
                    <div className="flex items-center justify-center gap-4 pt-1">
                        <button onClick={() => setShowMoreFilters(!showMoreFilters)} className="flex items-center gap-1.5 text-[10px] font-black text-[#0369A1] uppercase tracking-tight hover:underline">
                            {showMoreFilters ? <X size={12} /> : <PlusCircle size={12} />}
                            {showMoreFilters ? "Menos" : "Mais Filtros"}
                        </button>
                        <button onClick={limparFiltros} className="flex items-center gap-1.5 text-[10px] font-black text-red-600 uppercase tracking-tight hover:underline">
                            <X size={12} />
                            Limpar
                        </button>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-blue-200/50">
                        <div className="bg-[#0C4A6E]/10 border border-[#0C4A6E]/5 px-3 py-1 rounded-full flex items-center gap-1.5">
                            ...
                            <span className="text-[9px] font-black text-[#0C4A6E] uppercase tracking-tighter">BANCAS: {filteredDefesas.length}</span>
                        </div>
            ...
            <main className="max-w-7xl mx-auto px-4 py-4">
                {loading ? (
                    <div className="flex flex-col items-center justify-center py-10 text-slate-300">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-[2px]">Buscando suas bancas...</p>
                    </div>
                ) : filteredDefesas.length === 0 ? (
                    <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-slate-200">
                        <Search className="mx-auto mb-2 text-slate-200" size={32} />
                        <h3 className="text-sm font-bold text-slate-900">Você não possui bancas registradas</h3>
                        <p className="text-[10px] text-slate-400 uppercase font-bold mt-1">Nesta modalidade, você deve ser Orientador ou Avaliador</p>
                    </div>
                ) : (
                    <div className="space-y-1.5">
                        {filteredDefesas.map((defesa) => {
                            ...
                            let role = ''
                            if (defesa.orientador?.toLowerCase()?.trim() === userName) role = 'ORIENTADOR'
                            else if (defesa.coorientador?.toLowerCase()?.trim() === userName) role = 'COORIENTADOR'
                            else if (
                                defesa.avaliador1?.toLowerCase()?.trim() === userName ||
                                defesa.avaliador2?.toLowerCase()?.trim() === userName ||
                                defesa.avaliador3?.toLowerCase()?.trim() === userName
                            ) role = 'AVALIADOR'
                                ...
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {role && (
                                                <span className="text-[8px] font-black bg-blue-600 text-white px-1.5 py-0.5 rounded-full tracking-tighter">
                                                    {role}
                                                </span>
                                            )}
                                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
                        <div className="bg-[#E0F2FE] p-6 pb-12 relative">
                            <button onClick={() => setSelectedDefesa(null)} className="absolute right-6 top-6 w-8 h-8 rounded-full bg-white/50 flex items-center justify-center text-[#0C4A6E] hover:bg-white transition-all">
                                <X size={16} />
                            </button>
                            <div className="flex items-center gap-3 mb-2">
                                <span className="bg-[#0C4A6E] text-white px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest">{selectedDefesa.semestre}</span>
                            ...
                            <h2 className="text-[#0C4A6E] text-xl font-black uppercase leading-tight italic">{selectedDefesa.discente}</h2>
                        </div>
                            ...
                            <div className="space-y-6">
                                <section className="space-y-4">
                                    <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                                        ...
                                        <p className="text-xs font-bold text-slate-700 leading-relaxed italic">&quot;{selectedDefesa.titulo || 'TÍTULO NÃO INFORMADO'}&quot;</p>
                                    </div>
                                ...
                                <section className="space-y-3">
                                    ...
                                    <div className="space-y-2">
                                        <button onClick={handleGenerateAta} disabled={generatingPdf} className="w-full flex items-center gap-4 bg-blue-50/50 p-4 rounded-2xl border border-blue-100/50 hover:bg-blue-50 transition-all group">
                                        ...
                                        <button onClick={handleGenerateFolhaAprovacao} disabled={generatingPdf} className="w-full flex items-center gap-4 bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100/50 hover:bg-indigo-50 transition-all group">
                                        ...
                                        <button onClick={handleGenerateFichaFinal} disabled={generatingPdf} className="w-full flex items-center gap-4 bg-orange-50/50 p-4 rounded-2xl border border-orange-100/50 hover:bg-orange-50 transition-all group">
                                        ...
                                        <div className="space-y-2">
                                            <button onClick={() => setExpandFichas(!expandFichas)} className={clsx("w-full flex items-center justify-between bg-emerald-50/50 p-4 rounded-2xl border transition-all group", expandFichas ? "border-emerald-300 bg-emerald-50" : "border-emerald-100/50 hover:bg-emerald-50")}>
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 bg-emerald-100 rounded-xl flex items-center justify-center text-emerald-600 group-hover:scale-110 transition-transform"><Mic size={20} /></div>
                                                    <span className="text-[11px] font-black text-emerald-700 uppercase tracking-wider">Fichas Indiv.</span>
                                                </div>
                                                <div className={clsx("transition-transform duration-200 text-emerald-400", expandFichas ? "rotate-180" : "")}><ChevronDown size={18} /></div>
                                            </button>
                                            {expandFichas && (
                                                <div className="grid grid-cols-1 gap-1.5 pl-4 pr-1 py-1 animate-in slide-in-from-top-2 duration-200">
                                                    {[
                                                        ...
                                                        <button key={m.index} disabled={generatingPdf} onClick={async () => { setGeneratingPdf(true); try { await generateFichaIndividual(selectedDefesa as any, m.index); } finally { setGeneratingPdf(false); } }} className="w-full flex items-center justify-between p-3 bg-white border border-emerald-100 rounded-xl hover:bg-emerald-50 transition-colors group/sub">
                                        ...
                                        <button onClick={handleGenerateCertificados} disabled={generatingPdf} className="w-full flex items-center gap-4 bg-amber-50/50 p-4 rounded-2xl border border-amber-100/50 hover:bg-amber-50 transition-all group">
                                ...
                                <section className="space-y-3">
                                    ...
                                    <button onClick={() => { const link = `${window.location.origin}/defesas/avaliar?id=${selectedDefesa.id}`; navigator.clipboard.writeText(link); alert('Link de avaliação copiado!'); }} className="w-full flex items-center justify-center gap-2 bg-slate-50 text-slate-600 px-4 py-3 rounded-xl border border-slate-100 text-[10px] font-black uppercase hover:bg-slate-100 transition-all">
                                        <Share2 size={16} /> Copiar Link de Avaliação
                                    </button>
                                </section>
                            </div>
                        </div>
                        ...
                        <div className="p-6 bg-slate-50 border-t border-slate-100 flex items-center gap-3">
                            <Link href={`/defesas/avaliar?id=${selectedDefesa.id}`} className="flex-1 bg-[#0C4A6E] text-white py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-blue-100 hover:bg-[#0284C7] transition-all">
                                <Edit size={14} /> Lançar Notas
                            </Link>
                            <Link href={`/tcc/editar?id=${selectedDefesa.id}`} className="w-12 h-12 flex items-center justify-center bg-white border border-slate-200 rounded-2xl text-slate-400 hover:text-blue-600 hover:border-blue-200 transition-all shadow-sm" title="Editar Cadastro">
                                <FileText size={20} />
                            </Link>
                            <button onClick={() => handleDelete(selectedDefesa.id, selectedDefesa.discente)} className="w-12 h-12 flex items-center justify-center bg-white border border-red-100 rounded-2xl text-red-300 hover:text-red-500 hover:bg-red-50 transition-all shadow-sm" title="Excluir Banca">
                                <Trash2 size={20} />
                            </button>
                        </div>
                    </div>
